...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/08/18 14:39:17 by esnowpea          #+#    #+#             */
/*   Updated: 2020/08/28 17:14:26 by esnowpea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			img_triangle_put_part(t_img *img, t_point p1, t_point p2,
									t_point p3)
{
	t_point_d	p;
	t_color_d	color;
	...
	while ((int)round(p.x) != p2.x || (int)round(p.y) != p2.y)
	{
		...
		img_line_put(img, set_point((int)p.x, (int)p.y, set_color(color)), p3);
		if ((double)abs(p2.x - p1.x) > (double)abs(p2.y - p1.y))
			p = set_point_d(p.x + sign_x, p.y + (double)abs(p2.y - p1.y) /
					(double)abs(p2.x - p1.x) * sign_y, p1.color);
		else
			p = set_point_d(p.x + (double)abs(p2.x - p1.x) /
					(double)abs(p2.y - p1.y) * sign_x, p.y + sign_y, p1.color);
	}
}
...
void				img_triangle_put(t_img *img, t_point p1, t_point p2,
								t_point p3)
{
	img_triangle_put_part(img, p1, p2, p3);
	img_triangle_put_part(img, p2, p3, p1);
	img_triangle_put_part(img, p3, p1, p2);
}
